...
<head>
    ...
    <title><?php echo $_SITETITLE; ?> | Character</title>
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-8 offset-md-2">
					...
					<br>
<?php
		$checkAccount = sqlsrv_query($conn, "SELECT sUserID, sEmail, dDate FROM tUser WHERE nUserNo = ?;", array($_SESSION['nUserNo']), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
		...
		echo '<center><table width="100%;"><tr><th>Character Name</th><th>Level</th><th>Money</th><th>Class</th></tr>';
		while($fetchChar = sqlsrv_fetch_array($checkCharacter)){
			$checkClass = sqlsrv_query($conn, "SELECT nClass FROM World00_Character..tCharacterShape WHERE nCharNo = ?;", array($fetchChar['nCharNo']));
			$fetchClass = sqlsrv_fetch_array($checkClass);
			echo '<tr><th>'.$fetchChar['sID'].'</th><th>'.$fetchChar['nLevel'].'</th><th>'.getFiestaMoney($fetchChar['nMoney']).'</th><th>'.getClassToString($fetchClass['nClass']).'</th></tr>';
		}
		echo '</table></center>';
		echo '<br><hr><br>';
?>
