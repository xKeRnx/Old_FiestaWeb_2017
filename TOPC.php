...
<head>
    ...
    <title><?php echo $_SITETITLE; ?> | TOP Character</title>
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-md-8 offset-md-2">
                    <h2>TOP 100 Character</h2>
					<br>
<?php
		echo '<center><table width="100%">';
		echo '<tr><th>#</th><th>Username</th><th><a href="?order=nEXP">Level</a></th><th>Class</th><th>Guild</th></tr>';
		$i = 1;
		$order = 'ORDER BY nExp DESC';
		if(isset($_GET['order'])){
			switch($_GET['order']){
				case 'nEXP':
					$order = 'ORDER BY nExp DESC';
		...
		$getChar = sqlsrv_query($conn, "SELECT TOP 100 tCharacter.nCharNo, sID, nLevel, nExp, nPlayMin, nClass FROM World00_Character..tCharacter, World00_Character..tCharacterShape WHERE tCharacter.nCharNo = tCharacterShape.nCharNo AND bDeleted = 0 AND nAdminLevel = 0".$order.";", array(), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
		while($fetchChar = sqlsrv_fetch_array($getChar)){
			$selectGuild = sqlsrv_query($conn, "SELECT sName FROM World00_Character..tGuild WHERE nNo = (SELECT nNo FROM World00_Character..tGuildMember WHERE nCharNo = ?);", array($fetchChar['nCharNo']), array( "Scrollable" => SQLSRV_CURSOR_KEYSET ));
			$guild = 'None';
			if(sqlsrv_num_rows($selectGuild) == 1){
				$fetchGuild = sqlsrv_fetch_array($selectGuild);
				$guild = $fetchGuild['sName'];
			}
			echo '<tr><th>'.$i.'</th><th>'.$fetchChar['sID'].'</th><th>'.$fetchChar['nLevel'].'</th><th>'.getClassToString($fetchChar['nClass']).'</th><th>'.$guild.'</th></tr>';
			$i++;
		}
		echo '</table></center>';
